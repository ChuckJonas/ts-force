 ...
 */
export const queryAllComposite = async (
  query: string,
  opts: { restInstance?: Rest; allRows?: boolean; nextUrl?: string }
): Promise<SObject[]> => {
  opts = opts || {};
...
export const compositeRetrieve = <T extends Array<any>>(...sObjects: { [K in keyof T]: SObjectStatic<T[K]> }) => {
  return async (...queryFunctions: { [K in keyof T]: ((fields: FieldResolver<T[K]>) => SOQLQueryParams) | string }) => {
    let c = new CompositeBatch();
    let queryResults = [];
    let nextRecordQueries: Array<{
      nextRecordsUrl: string;
      handleResults: (r: CompositeBatchResult<QueryResponse<any>, StandardRestError[]>) => void;
    }> = [];
